...
func TestResizeCluster3to5(t *testing.T) {
	...
	e2eutil.LogfWithTimestamp(t, "reached to %d galera nodes", replicas)
	...
	updateFunc := func(galera *apigalera.Galera) {
		galera = e2eutil.GaleraSize(galera, updateReplicas)
	}

	if _, err = e2eutil.UpdateGalera(f.GaleraClient, testGalera.Name, testGalera.Namespace, 3, updateFunc); err != nil {
		t.Fatalf("failed to resize galera cluster")
	}

	curGalera, err := f.GaleraClient.SqlV1beta2().Galeras(f.Namespace).Get(testGalera.Name, metav1.GetOptions{})
	if err != nil {
		t.Fatalf("failed to retrieve galera CR: %v", err)
	}

	if _, err := e2eutil.WaitUntilSizeReached(t, f.GaleraClient, updateReplicas, 12, curGalera); err != nil {
		t.Fatalf("failed to update %d -> %d members galera cluster: %v", replicas, updateReplicas, err)
	}

	ok, err = e2eutil.MatchGaleraSize(f.KubeClient, f.Config, testGalera, updateReplicas)
	if err != nil || ok == false {
		t.Fatalf("failed to have wsrep_cluster_size equals to %d : %v", updateReplicas, err)
	}

	e2eutil.LogfWithTimestamp(t, "checking data on all nodes")
	ok, err = e2eutil.CheckData(f.KubeClient, f.Config, curGalera)
	if err != nil || ok == false {
		t.Fatalf("failed to check data : %s", err)
	}

	e2eutil.LogfWithTimestamp(t, "reached to %d galera nodes with image %s", updateReplicas, galeraImage)
}
